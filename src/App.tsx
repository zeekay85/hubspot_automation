...
function App() {
  ...
  return (
    <div className="min-h-screen text-ink transition-colors duration-300 dark:text-slate-100 lg:flex">
      ...
      <main className="flex-1 p-4 sm:p-8 lg:p-10">
        ...
        <section className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white/90 shadow-soft backdrop-blur transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900/85">
          <div className="p-7 sm:p-9 lg:p-11">
            <div className="max-w-3xl">
              <p className="text-xs font-bold uppercase tracking-[0.24em] text-brand-600">
                Portfolio dashboard scaffold
              </p>
              <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-ink dark:text-white sm:text-5xl">
                Marketing Ops Hub
              </h1>
              <p className="mt-4 max-w-2xl text-base leading-7 text-muted dark:text-slate-300">
        ...
        <section className="mt-9">
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-extrabold tracking-tight text-ink dark:text-white">{selectedTool.title}</h2>
            </div>
            <p className="max-w-xl text-sm leading-6 text-muted dark:text-slate-300">{selectedTool.description}</p>
          </div>
          {renderWorkspace()}
        </section>
      </main>
